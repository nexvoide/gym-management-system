...
import { db } from "@/db";
import { auditLogs, invoiceItems, invoices, members, membershipHistory, membershipPlans, memberships, trainers } from "@/db/schema";
...
async function standardPlan(gymId: string) {
    ...
    if (!plan) {
        await db.insert(membershipPlans).values({ id: crypto.randomUUID(), gymId, name: "Standard", description: "Default membership for new members", durationDays: 30, price: 0, active: true }).onConflictDoNothing();
...
export async function createMember(_: MemberFormState, formData: FormData): Promise<MemberFormState> {
    ...
    try {
        await db.transaction(async (tx) => {
            ...
            const memberNumber = `FM-${sequence}`, startsAt = new Date(), endsAt = addDays(startsAt, plan.durationDays), membershipId = crypto.randomUUID();
            await tx.insert(members).values({ id, gymId: user.gymId, memberNumber, ...parsed.data, profilePhotoUrl: photoPath });
            await tx.insert(memberships).values({ id: membershipId, gymId: user.gymId, memberId: id, planId: plan.id, status: "active", startsAt, endsAt, basePrice: plan.price, discount: 0, finalPrice: plan.price, createdBy: user.id });
            ...
            if (plan.price > 0) {
                ...
                await tx.insert(invoices).values({ id: invoiceId, gymId: user.gymId, memberId: id, membershipId, invoiceNumber, issuedAt: startsAt, dueAt: startsAt, subtotal: plan.price, total: plan.price, paid: 0, balance: plan.price, status: "unpaid" });
