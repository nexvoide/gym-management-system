...
export default async function Page({ params }: {
    ...
    redirect(`/payments/invoices/${id}`); return <div className="content content-narrow"><div className="page-head"><div><div className="eyebrow">{invoice.invoiceNumber}</div><h1>Record payment</h1><p>Receive payment from {invoice.firstName} {invoice.lastName}.</p></div></div><PaymentForm action={recordPayment.bind(null, id)} invoiceId={id} balance={invoice.balance} currency={invoice.currency} methods={await paymentMethods(user.gymId)}/></div>; }
