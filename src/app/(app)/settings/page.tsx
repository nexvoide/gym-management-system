...
const skins = [
  { id: "midnight", name: "Midnight", colors: ["#090d0f", "#1b2024", "#b7ff00"] },
  { id: "slate", name: "Slate", colors: ["#111827", "#202b3a", "#60a5fa"] },
  { id: "light", name: "Light", colors: ["#f4f6f8", "#ffffff", "#2563eb"] },
] as const;
  ...
  return <div className="content settings-page">
    ...
    <div className="settings-grid"><aside className="card settings-menu"><a className="settings-link active" href="#profile">Gym profile</a><a className="settings-link" href="#branding">Branding</a><a className="settings-link" href="#appearance">Appearance</a><a className="settings-link" href="#communications">Communications</a>{can(user.role, "users.manage") && <Link className="settings-link" href="/settings/staff">Staff & roles</Link>}</aside>
      <div className="settings-stack">
        ...
        <form id="appearance" className="card form-card" action={updateAppearance}><h3>Appearance</h3><p>Choose one controlled professional skin for this gym.</p><div className="skin-grid">{skins.map((skin) => <label className={`skin-choice ${gym.skin === skin.id ? "active" : ""}`} key={skin.id}><input type="radio" name="skin" value={skin.id} defaultChecked={gym.skin === skin.id} disabled={!owner}/><span className="skin-preview" style={{ background: skin.colors[0] }}><i style={{ background: skin.colors[1] }}/><b style={{ background: skin.colors[2] }}/></span><strong>{skin.name}</strong><small>{gym.skin === skin.id ? "✓ Active" : "Select skin"}</small></label>)}</div><div className="save-row"><button className="btn btn-primary" disabled={!owner}>Apply appearance</button></div></form>
