...
export const gyms = pgTable("gyms", {
  ...
  locale: text("locale").notNull().default("en"), ...timestamps,
});
...
export const membershipPlans = pgTable("membership_plans", {
  ...
  durationDays: integer("duration_days").notNull(), price: numeric("price", { precision: 14, scale: 2, mode: "number" }).notNull(), accessDescription: text("access_description"), notes: text("notes"),
...
export const memberships = pgTable("memberships", {
  ...
  status: text("status",{enum:["active","expired","frozen","cancelled"]}).notNull(), startsAt: timestamp("starts_at", { withTimezone: true, mode: "date" }).notNull(), endsAt: timestamp("ends_at", { withTimezone: true, mode: "date" }).notNull(),
  basePrice: numeric("base_price", { precision: 14, scale: 2, mode: "number" }).notNull().default(0), discount: numeric("discount", { precision: 14, scale: 2, mode: "number" }).notNull().default(0), finalPrice: numeric("final_price", { precision: 14, scale: 2, mode: "number" }).notNull().default(0), notes:text("notes"),
...
export const invoices=pgTable("invoices",{
  ...
  issuedAt:timestamp("issued_at", { withTimezone: true, mode: "date" }).notNull(),dueAt:timestamp("due_at", { withTimezone: true, mode: "date" }).notNull(),subtotal:numeric("subtotal",{precision:14,scale:2,mode:"number"}).notNull(),discount:numeric("discount",{precision:14,scale:2,mode:"number"}).notNull().default(0),tax:numeric("tax",{precision:14,scale:2,mode:"number"}).notNull().default(0),total:numeric("total",{precision:14,scale:2,mode:"number"}).notNull(),paid:numeric("paid",{precision:14,scale:2,mode:"number"}).notNull().default(0),balance:numeric("balance",{precision:14,scale:2,mode:"number"}).notNull(),status:text("status",{enum:["paid","partially_paid","unpaid","overdue","refunded"]}).notNull(),notes:text("notes"),...timestamps,
...
export const invoiceItems=pgTable("invoice_items",{
  id:text("id").primaryKey(),invoiceId:text("invoice_id").notNull().references(()=>invoices.id),description:text("description").notNull(),quantity:numeric("quantity",{precision:12,scale:3,mode:"number"}).notNull().default(1),unitPrice:numeric("unit_price",{precision:14,scale:2,mode:"number"}).notNull(),amount:numeric("amount",{precision:14,scale:2,mode:"number"}).notNull(),createdAt:timestamp("created_at", { withTimezone: true, mode: "date" }).notNull().$defaultFn(()=>new Date()),
},(t)=>[index("invoice_items_invoice_idx").on(t.invoiceId)]);
export const payments=pgTable("payments",{
  id:text("id").primaryKey(),gymId:text("gym_id").notNull().references(()=>gyms.id),memberId:text("member_id").notNull().references(()=>members.id),invoiceId:text("invoice_id").notNull().references(()=>invoices.id),amount:numeric("amount",{precision:14,scale:2,mode:"number"}).notNull(),method:text("method").notNull(),paidAt:timestamp("paid_at", { withTimezone: true, mode: "date" }).notNull(),reference:text("reference"),notes:text("notes"),recordedBy:text("recorded_by").references(()=>users.id),createdAt:timestamp("created_at", { withTimezone: true, mode: "date" }).notNull().$defaultFn(()=>new Date()),
...
export const expenses=pgTable("expenses",{
  id:text("id").primaryKey(),gymId:text("gym_id").notNull().references(()=>gyms.id),categoryId:text("category_id").notNull().references(()=>expenseCategories.id),description:text("description").notNull(),amount:numeric("amount",{precision:14,scale:2,mode:"number"}).notNull(),expenseDate:timestamp("expense_date", { withTimezone: true, mode: "date" }).notNull(),paymentMethod:text("payment_method").notNull(),vendor:text("vendor"),receiptUrl:text("receipt_url"),notes:text("notes"),createdBy:text("created_by").notNull().references(()=>users.id),...timestamps,
