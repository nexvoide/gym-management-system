...
test("staff setup clears an existing owner session before staff signup",()=>{const setup=readFileSync("src/app/set-password/actions.ts","utf8");assert.ok(setup.indexOf("supabase.auth.signOut()")<setup.indexOf("supabase.auth.signUp("));});
test("staff setup does not accept an existing Supabase identity as a new password",()=>{const setup=readFileSync("src/app/set-password/actions.ts","utf8");assert.match(setup,/authData\.user\?\.identities\?\.length/);assert.match(setup,/Use Forgot password/);assert.match(setup,/mustChangePassword:\s*false/);});
