...
import { expiryMessage, welcomeMessage, whatsappExpiryUrl } from "../src/lib/member-communications";
...
test("Phase 10.2 UI and durable idempotency are wired", async () => {
  const [settings, schema, layout] = await Promise.all([
    readFile("src/app/(app)/settings/page.tsx", "utf8"), readFile("src/db/schema.ts", "utf8"), readFile("src/app/(app)/layout.tsx", "utf8"),
  ]);
